                    ...
                    {% if product.available and product.display_price %}
                        <div class="col-8 col-md-6 pr-1">
                            ...
                            {% if product.isSubscribable() or product.variations %}
                                <a href="{{ quickshop_href }}" class="{{ quickshop_button_js_classes }} {{ quickshop_button_primary_classes }} d-block item-buy-open" title="{{ 'Compra rápida de' | translate }} {{ product.name }}" aria-label="{{ 'Compra rápida de' | translate }} {{ product.name }}" {% if not product.isSubscribable() %}data-component="product-list-item.add-to-cart" data-component-value="{{ product.id }}"{% endif %}>{{ 'Comprar' | translate }}</a>
                            {% else %}
                                <form class="js-product-form" method="post" action="{{ store.cart_url }}">
                                    <input type="hidden" name="add_to_cart" value="{{product.id}}" />
                                    {% set state = store.is_catalog ? 'catalog' : (product.available ? product.display_price ? 'cart' : 'contact' : 'nostock') %}
