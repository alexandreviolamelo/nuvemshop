...
.slider-video-container.desktop-ratio {
    padding-bottom: 39.0625%; /* 1920x750 Aspect Ratio */
}
.slider-video-container.mobile-ratio {
    padding-bottom: 121.95%; /* 820x1000 Aspect Ratio */
}
						...
						<div class="swiper-wrapper">
							...
							{% for slide in slider %}
								<div class="swiper-slide slide-container">
                                    {% set is_video = "VIDEO::" in slide.link or ("VIDEO::" in slide.description) %}
                                    {% if is_video %}
                                        {% if "VIDEO::" in slide.link %}
                                            {% set raw_video_url = slide.link %}
                                        {% else %}
                                            {% set raw_video_url = slide.description %}
                                        {% endif %}
                                        {% set video_url = raw_video_url | replace("VIDEO::", "") %}
                                        {% if "youtube.com/watch?v=" in video_url or "youtu.be/" in video_url %}
                                            {% if "youtube.com/watch?v=" in video_url %}
                                                {% set video_id = video_url | split('v=') | last %}
                                            {% else %}
                                                {% set video_id = video_url | split('/') | last %}
                                            {% endif %}
                                            {% set embed_url = "https://www.youtube.com/embed/" ~ video_id ~ "?autoplay=1&mute=1&controls=0&showinfo=0&modestbranding=1&rel=0&loop=1&playlist=" ~ video_id %}
                                        {% elseif "vimeo.com/" in video_url %}
                                            {% set video_id = video_url | split('/') | last %}
                                            {% set embed_url = "https://player.vimeo.com/video/" ~ video_id ~ "?background=1&autoplay=1" %}
                                        {% else %}
                                            {% set embed_url = video_url %}
                                        {% endif %}
                                        <div class="slider-video-container {% if mobile %}mobile-ratio{% else %}desktop-ratio{% endif %}">
                                            <iframe src="{{ embed_url }}" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>
                                        </div>
                                    {% else %}
                                        {% if slide.link %}
                                        ...
                                        <div class="slider-slide{% if has_auto_height_slider %} position-relative h-auto{% endif %}">
                                            ...
                                            {% if has_text %}
                                                <div class="swiper-text swiper-{{ slide.color }}">	
                                                    {% if slide.description and not is_video %}
                                                    ...
                                                    {% endif %}
                                                    {% if slide.button and slide.link and not is_video %}
