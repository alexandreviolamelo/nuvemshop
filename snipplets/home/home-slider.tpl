									...
									{% if is_video %}
										<div class="slide-video-wrapper">
											{% if 'youtube.com' in video_url or 'youtu.be' in video_url %}
												...
												<iframe src="https://www.youtube.com/embed/{{ video_id }}?autoplay=1&mute=1&loop=1&playlist={{ video_id }}&controls=0&modestbranding=1&showinfo=0&rel=0" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>
											{% elseif 'vimeo.com' in video_url %}
												{% set video_id = video_url | split('/') | last | split('?') | first %}
												<iframe src="https://player.vimeo.com/video/{{ video_id }}?autoplay=1&loop=1&autopause=0&muted=1&background=1" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>
											{% endif %}
										</div>
									{% else %}
										<div class="slider-slide{% if has_auto_height_slider %} position-relative h-auto{% endif %}">
											{% set apply_lazy_load = settings.home_order_position_0 != 'slider' or not (loop.first and ((has_main_slider and not has_mobile_slider) or (has_mobile_slider and mobile))) %}
											...
											{% endif %}
											<img {% if not apply_lazy_load %}fetchpriority="high"{% endif %} {% if slide.width and slide.height %} width="{{ slide.width }}" height="{{ slide.height }}" {% endif %} {% if apply_lazy_load %}data-{% endif %}src="{{ slide_src }}" {% if apply_lazy_load %}data-{% endif %}srcset="{% if use_smaller_thumbs %}{{ slide_image | settings_image_url('large') }} 480w, {{ slide_image | settings_image_url('huge') }} 640w, {% endif %}{{ slide_image | settings_image_url('original') }} 1024w, {{ slide_image | settings_image_url('1080p') }} 1920w" class="slider-image {% if apply_lazy_load %}swiper-lazy fade-in{% endif %}{% if has_auto_height_slider %} d-block{% endif %}" alt="{{ 'Carrusel' | translate }} {{ loop.index }}">
											<div class="placeholder-fade"></div>
										</div>
									{% endif %}
									
									{# Text can be displayed over videos OR images #}
									{% set has_text = slide.title or clean_description or (slide.button and slide.link) %}
									{% if has_text %}
										<div class="swiper-text swiper-{{ slide.color }}">	
											{% if clean_description %}
												<div class="swiper-description {% if has_auto_height_slider and not has_mobile_slider %}font-small font-md-normal mb-md-3 mb-2{% else %}mb-3{% endif %}">{{ clean_description }}</div>
											{% endif %}
											{% if slide.title %}
